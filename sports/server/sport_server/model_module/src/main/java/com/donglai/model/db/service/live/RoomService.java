...
public class RoomService {
    ...
    public boolean userIsLive(String userId){
        User user = userService.findById(userId);
        if(Objects.isNull(user) || !user.isLiveUser()) return false;
        Room room = findByLiveUserId(user.getLiveUserId());
        if(Objects.isNull(room)) return false;
        return redisTemplate.opsForSet().isMember(LIVING_ROOM, room.getId());
    }
