...
import static com.donglai.web.response.GlobalResponseCode.USER_NOT_FOUND;
...
public class BackOfficeUserBackServiceImpl implements BackOfficeUserBackService {
    ...
    public RestResponse updateUserEnable(UpdateUserEnableRequest request) {
        ...
        if(backOfficeUsers.size()!=request.getIds().size())  return new ErrorResponse(USER_NOT_FOUND);
