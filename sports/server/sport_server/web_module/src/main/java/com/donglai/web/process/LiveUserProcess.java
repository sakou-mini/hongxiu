...
public class LiveUserProcess {
    ...
    public RestResponse userDetailById(String userId){
        if(StringUtils.isNullOrBlank(userId)) return new ErrorResponse(GlobalResponseCode.PARAM_ERROR);
        User user = userService.findById(userId);
        if(Objects.isNull(user)) return  new ErrorResponse(GlobalResponseCode.USER_NOT_FOUND);
        return new SuccessResponse().withData(user);
    }
}
