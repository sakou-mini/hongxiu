...
docker stop hongxiu_account_module
docker rm hongxiu_account_module
cd ../
docker build -t hongxiu_account_image -f DockerFileAccountModule .
...
-w /usr/src/app \
--network hongxiu-network --name hongxiu_account_module hongxiu_account_image \
