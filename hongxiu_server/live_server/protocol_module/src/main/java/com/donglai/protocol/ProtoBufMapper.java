...
public class ProtoBufMapper {
    ...
    private static String getTopic(String messageName) {
        ...
        } else if (messageName.startsWith(HongXiu.TopicEnum.BLOGS.name())) {
            return HongXiu.TopicEnum.BLOGS.name();
        } else if (messageName.startsWith(HongXiu.TopicEnum.STATISTICS.name())) {
            return HongXiu.TopicEnum.STATISTICS.name();
        }
        return "";
    }
    ...
    private static String getMessagePackageName(String messageName) {
        ...
        List<String> blogsMessages = Blog.getDescriptor().getMessageTypes().stream().map(type -> type.getFullName().toUpperCase()).collect(Collectors.toList());
        ...
        } else if (blogsMessages.contains(messageName)) {
            return Blog.class.getName().split("\\$")[0];
        }
        return "";
    }
