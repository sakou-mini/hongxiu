...
public class QueueProcess {
    ...
    public static HongXiu.HongXiuMessageRequest getExecuteFinishPb(QueueExecute queueExecute) {
        ...
        switch (queueExecute.getFromServer()) {
            case QueueExecute.BLOG:
                /*1.构造返回的Blgos Builder*/
                return HongXiu.HongXiuMessageRequest.newBuilder().setBlogsOfQueueFinishRequest(Common.BlogsOfQueueFinishRequest.newBuilder().setQueueId(queueExecute.getId())).build();
            ...
            case QueueExecute.STATISTIC:
                /*1.构造返回的Live Builder*/
                return HongXiu.HongXiuMessageRequest.newBuilder().setStatisticsOfQueueFinishRequest(Common.StatisticsOfQueueFinishRequest.newBuilder().setQueueId(queueExecute.getId())).build();
            default:
                return null;
        }
    }
